...
public class RentalMailService { // 郵件失敗不傳回付款交易。
    ...
    /** 寄送場地租借正式付款成功通知。 */
    public void send(Map<String,Object> rental, int amount, String trade) {
        send(rental, amount, trade, false);
    }

    /** 寄送場地租借展示付款成功通知。 */
    public void sendDemo(Map<String,Object> rental, int amount, String trade) {
        send(rental, amount, trade, true);
    }

    /** 寄送場地租借付款成功通知（區分正式與展示付款）。 */
    public void send(Map<String,Object> rental, int amount, String trade, boolean isDemo) { // 此方法只在付款成功提交後呼叫。
        try { // 所有寄信與查詢錯誤均不得影響付款回呼。
            ...
            message.setSubject("【星澄飯店】場地租借付款成功"); // 使用指定郵件標題。
            String paymentMethod = isDemo ? "展示用模擬付款" : "信用卡 / 綠界 Stage";
            String tradeLabel = isDemo ? "展示交易編號" : "綠界交易編號";
            message.setText("您好，"+profile.getName()+"：\n租借編號："+rental.get("rental_id")+"\n場地："+rental.get("venue_name")+"\n活動："+rental.get("event_name")+"\n日期："+rental.get("rental_date")+"\n人數："+rental.get("guest_count")+"\n付款金額：NT$"+amount+"\n付款方式："+paymentMethod+"\n付款狀態：已付款\n租借狀態：已確認\n"+tradeLabel+"："+trade); // 只包含使用者可見資訊及歷史金額。
