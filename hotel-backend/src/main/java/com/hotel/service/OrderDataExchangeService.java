...
public class OrderDataExchangeService {
    ...
    for (CustomerOrder order : orders) {
      ...
      orderDTOs.add(
          OrderExportItemDTO.builder()
              ...
              .couponCode(
                  order.getCouponId() != null ? couponCodeMap.get(order.getCouponId()) : null)
              .paymentId(order.getPaymentId())
              .paymentTransactionId(
                  order.getPaymentId() != null ? paymentTxMap.get(order.getPaymentId()) : null)
              .orderStatus(order.getOrderStatus())
              .items(itemDTOs)
              .build());
    }

    OrderDataExchangeDTO exchangeDTO =
        OrderDataExchangeDTO.builder()
            .coupons(coupons)
            .payments(payments)
            .orders(orderDTOs)
            .build();
  ...
  public int importFromJson(InputStream inputStream) throws Exception {
    ...
    if (payload.getCoupons() != null) {
      for (Coupon c : payload.getCoupons()) {
        ...
        Coupon existing =
            couponRepository.findByCouponCode(c.getCouponCode().trim().toUpperCase()).orElse(null);
        if (existing == null) {
          ...
          newCoupon.setEndDate(
              c.getEndDate() != null ? c.getEndDate() : LocalDateTime.now().plusMonths(1));
    ...
    if (payload.getPayments() != null) {
      for (Payment p : payload.getPayments()) {
        ...
        Payment existing =
            paymentRepository.findByTransactionId(p.getTransactionId().trim()).orElse(null);
        if (existing == null) {
          ...
          newPayment.setPaymentStatus(
              p.getPaymentStatus() != null ? p.getPaymentStatus() : "PENDING");
          newPayment.setPaymentTime(p.getPaymentTime());
          newPayment.setCreatedAt(
    ...
    if (payload.getOrders() != null) {
      for (OrderExportItemDTO orderItemDTO : payload.getOrders()) {
        ...
        order.setOrderDate(
            orderItemDTO.getOrderDate() != null
                ? orderItemDTO.getOrderDate()
                : LocalDateTime.now());
        order.setOriginalAmount(
            orderItemDTO.getOriginalAmount() != null ? orderItemDTO.getOriginalAmount() : 0);
        order.setDiscountAmount(
            orderItemDTO.getDiscountAmount() != null ? orderItemDTO.getDiscountAmount() : 0);
        order.setFinalAmount(
            orderItemDTO.getFinalAmount() != null ? orderItemDTO.getFinalAmount() : 0);
        order.setOrderStatus(
            orderItemDTO.getOrderStatus() != null ? orderItemDTO.getOrderStatus() : "PENDING");

        // 解析 couponId
        if (orderItemDTO.getCouponCode() != null && !orderItemDTO.getCouponCode().isBlank()) {
          couponRepository
              .findByCouponCode(orderItemDTO.getCouponCode().trim().toUpperCase())
        ...
        // 解析 paymentId
        if (orderItemDTO.getPaymentTransactionId() != null
            && !orderItemDTO.getPaymentTransactionId().isBlank()) {
          paymentRepository
              .findByTransactionId(orderItemDTO.getPaymentTransactionId().trim())
  ...
  public void exportCouponsToCsv(Writer writer) {
    ...
    pw.write('\ufeff');
    pw.println(
        "coupon_code,coupon_name,discount_type,discount_value,minimum_amount,start_date,end_date,status");
  ...
  public int importCouponsFromCsv(InputStream inputStream) throws Exception {
    ...
    try (BufferedReader reader =
      ...
      while ((line = reader.readLine()) != null) {
        ...
        int minAmount =
            cols.length > 4 && !cols[4].isBlank() ? Integer.parseInt(cols[4].trim()) : 0;
        LocalDateTime startDate =
            cols.length > 5 && !cols[5].isBlank()
                ? LocalDateTime.parse(cols[5].trim(), dtf)
                : LocalDateTime.now();
        LocalDateTime endDate =
            cols.length > 6 && !cols[6].isBlank()
                ? LocalDateTime.parse(cols[6].trim(), dtf)
                : LocalDateTime.now().plusMonths(3);
        String status =
            cols.length > 7 && !cols[7].isBlank() ? cols[7].trim().toUpperCase() : "ACTIVE";
