...
public class MailUtil {
    ...
    public void sendOrderConfirmation(String toEmail, String orderId, int amount) {
        ...
        // Example store and address info (should be derived from order in real
        // implementation)
        String storeName = "星澄飯店 (示例分店)";
        String deliveryAddress = "桃園市中壢區新生路二段421-99號";
        String mapLink = "https://www.google.com/maps/search/"
                + java.net.URLEncoder.encode(deliveryAddress, java.nio.charset.StandardCharsets.UTF_8);
        String subject = "【星澄飯店】訂單確認 - 訂單編號 " + orderId;
        // Build HTML email using StringBuilder for readability and maintainability
        StringBuilder sb = new StringBuilder();
        sb.append(
                "<!DOCTYPE html><html><body style='margin:0;padding:0;background-color:#f4f1ea;font-family:\"Microsoft JhengHei\",Arial,sans-serif;color:#333;'>");
        sb.append(
                "<table role='presentation' width='100%' cellpadding='0' cellspacing='0' style='background:#f4f1ea;padding:30px 10px;'><tr><td align='center'>");
        sb.append(
                "<table role='presentation' width='100%' style='max-width:560px;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 15px rgba(0,0,0,0.08);' cellpadding='0' cellspacing='0'>");
        sb.append(
                "<tr><td style='background:linear-gradient(135deg,#2b2219 0%,#4a3b2a 100%);padding:32px 24px;text-align:center;'><h1 style='margin:0;color:#d4af37;font-size:26px;letter-spacing:3px;font-weight:bold;'>星澄飯店</h1>");
        sb.append(
                "<p style='margin:6px 0 0;color:#eae5dc;font-size:13px;letter-spacing:1px;'>Grand Aster Hotel &amp; Resorts</p></td></tr>");
        sb.append("<tr><td style='padding:36px 32px;'>");
        sb.append(
                "<h2 style='margin-top:0;color:#4a3b2a;font-size:20px;font-weight:bold;border-bottom:2px solid #e8dfd3;padding-bottom:12px;'>訂單確認</h2>");
        sb.append("<p style='font-size:15px;line-height:1.7;color:#555;margin:18px 0 10px;'>您好，</p>");
        sb.append("<p style='font-size:15px;line-height:1.7;color:#555;margin:0 0 24px;'>我們已收到您在 <strong>" + storeName
                + "</strong> 的訂單，實際出貨狀況依各店家接單狀況為主。</p>");
        sb.append("<ul style='list-style:none;padding:0;margin:0 0 20px;'><li>訂單編號：<b>" + orderId + "</b></li><li>下單時間："
                + orderTime + "</li><li>取餐/送達地址：<a href='" + mapLink + "' style='color:#2b2219;'>" + deliveryAddress
                + "</a></li></ul>");
        sb.append(
                "<table width='100%' cellpadding='5' cellspacing='0' style='border-collapse:collapse;'><thead><tr style='background:#faf7f2;'><th align='left'>商品</th><th align='right'>金額</th></tr></thead><tbody>");
        sb.append("<tr><td>1 x 訂單商品範例</td><td align='right'>$ " + amount + "</td></tr>");
        sb.append("</tbody></table>");
        sb.append("<div style='margin-top:20px;text-align:right;font-size:15px;color:#4a3b2a;'><p>小計：<b>$ " + amount
                + "</b></p><p>外送服務費：<b>$ 0</b></p><p>平台費：<b>$ 0</b></p><p>稅金 (5%)：<b>$ 0</b></p><hr style='border:none;border-top:1px solid #eee;margin:10px 0;'/><p style='font-size:18px;color:#d4af37;'>訂單總額：<b>$ "
                + amount + "</b></p></div>");
        sb.append("<p style='text-align:center;margin:30px 0;'><a href='https://example.com/tracking?order=" + orderId
                + "' style='display:inline-block;background:#2b2219;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;'>追蹤您的訂單</a></p>");
        sb.append("<p style='font-size:12px;color:#999;line-height:1.6;margin:0;'>需要協助嗎？請前往客服中心。</p>");
        sb.append(
                "</td></tr><tr><td style='background:#fbf9f6;padding:18px 24px;text-align:center;border-top:1px solid #f0ebe1;'><p style='margin:0;font-size:12px;color:#8c8275;'>※ 此為系統自動發送信件，請勿直接回覆。</p></td></tr></table></td></tr></table></body></html>");
